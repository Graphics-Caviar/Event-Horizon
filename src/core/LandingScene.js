...
export class LandingScene {
  constructor(sceneManager, assetManager, { shipKey = 'raven', pilotKey = 'zara', onComplete = () => {} } = {}) {
  ...
  _buildLandingPad() {
    ...
    for (let i = 0; i < beaconCount; i++) {
      const angle = (i / beaconCount) * Math.PI * 2 + (Math.PI / 8);
      ...
      const beaconLight = new THREE.PointLight(0x4de3ff, 0.6, 4);
      beaconLight.position.set(bx, 0.7, bz);
      this.landingPadGroup.add(beaconLight);

      this.beacons.push({ mesh: beacon, light: beaconLight, material: beaconMat, offset: i * 0.4 });
    }
  ...
  _buildSpaceship() {
    this.ship = new Spaceship(this.group, { colors: this.shipConfig.colors });
  ...
  update(delta) {
    ...
    if (t < 3.2) {
      ...
      // Ship position
      const startX = 0, startY = 24, startZ = -50;
      const midX = 0, midY = 7.5, midZ = -14;

      this.ship.group.position.x = THREE.MathUtils.lerp(startX, midX, ease);
      this.ship.group.position.y = THREE.MathUtils.lerp(startY, midY, ease);
      this.ship.group.position.z = THREE.MathUtils.lerp(startZ, midZ, ease);
      ...
      // Full throttle burn
      this.ship.update(delta, { throttle: 0.95, turnInput: 0 });
      ...
      const midX = 0, midY = 7.5, midZ = -14;
      const landX = 0, landY = 1.4, landZ = 0;

      this.ship.group.position.x = THREE.MathUtils.lerp(midX, landX, ease);
      this.ship.group.position.y = THREE.MathUtils.lerp(midY, landY, ease);
      this.ship.group.position.z = THREE.MathUtils.lerp(midZ, landZ, ease);
      ...
      const throttle = THREE.MathUtils.lerp(0.85, 0.45, progress);
      ...
      const bounce = Math.sin(progress * Math.PI) * 0.08 * (1 - progress);

      this.ship.group.position.set(0, THREE.MathUtils.lerp(startY, finalY, ease) + bounce, 0);
      this.ship.group.rotation.set(0, 0, 0);

      // Retro-thrusters wind down
      const throttle = THREE.MathUtils.lerp(0.4, 0.05, progress);
      this.ship.update(delta, { throttle, turnInput: 0 });
      ...
      if (this.dustParticles.material.opacity > 0.01) {
        this.dustParticles.material.opacity = Math.max(0, this.dustParticles.material.opacity - delta * 0.5);
      }
  ...
  _updateDustParticles(delta, intensity) {
    ...
    this.dustParticles.material.opacity = Math.min(0.65, intensity * 0.7);
