...
export default class StatsPanel {
  ...
  constructor(characters = CHARACTERS) {
    ...
    this._barTracks = [...this._el.querySelectorAll('[data-bar-id]')];
    this._legendItems = [...this._el.querySelectorAll('[data-legend-id]')];
  }
  ...
  setEmphasis(id) {
    for (const track of this._barTracks) {
      ...
      track.classList.toggle('is-emphasized', Boolean(id) && isTarget);
      track.classList.toggle('is-dimmed', Boolean(id) && !isTarget);
    }
    for (const item of this._legendItems) {
      const isTarget = item.dataset.legendId === id;
      item.classList.toggle('is-emphasized', Boolean(id) && isTarget);
      item.classList.toggle('is-dimmed', Boolean(id) && !isTarget);
    }
  }
}
