...
export default class CharacterModel {
  ...
  async _load() {
    try {
      const gltf = await loadGLTF(this.character.model, this._onProgress);
      this._onModelLoaded(gltf);
    } catch (err) {
      console.warn(`[CharacterModel:${this.character.id}] ${err.message}`);
  ...
  _buildErrorFallback() {
    ...
    const { sprite, texture, material } = makeLabelSprite('MODEL UNAVAILABLE', this.character.themeColor);
  ...
  update(dt, elapsed) {
    this._currentScale = damp(this._currentScale, this._targetScale, 6, dt);
    ...
    switch (this.state) {
      ...
      case 'ready':
        if (this.mixer) {
          ...
          this._applyProceduralIdle(this.model, elapsed, 0);
        }
        break;

      case 'error':
        this._applyProceduralIdle(this._fallback, elapsed, 0);
