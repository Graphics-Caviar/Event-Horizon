...
export class CharacterSystem {
	...
	constructor(scene) {
		...
		for (const character of CHARACTERS) {
			...
			if (!slot) {
				console.warn(
					`[CharacterSystem] no scene slot found for "${character.id}" — skipping`
				)
				continue
			}
			...
			const model = new CharacterModel(character, {
				onReady: () => {
					console.log(
						`[CharacterSystem] "${character.id}" model ready`
					)
				},
				onError: (err) => {
					console.warn(
						`[CharacterSystem] "${character.id}" using fallback: ${err.message}`
					)
				},
			})
	...
	setHovered(id) {
		...
		if (id && !this._selectedId) {
			this._models
				.get(id)
				?.play('ability', { fade: 0.25, loop: false })
		}
	...
	_refreshEmphasis() {
		for (const [id, model] of this._models) {
			...
			const isHovered =
				id === this._hoveredId && !this._selectedId
			const isDimmed =
				Boolean(this._selectedId) && !isSelected
