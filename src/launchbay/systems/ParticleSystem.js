...
export class ParticleSystem {
  ...
  _buildNebula() {
    ...
    const nebulaConfigs = [
      { color: '#1c3f66', position: [-14, 3, -22], scale: 26, opacity: 0.35 },
      { color: '#2a1740', position: [16, -2, -26], scale: 30, opacity: 0.3 },
      { color: '#0f3325', position: [2, 8, -30], scale: 22, opacity: 0.22 },
    ];
  ...
  update(dt, elapsed) {
    ...
    for (let i = 0; i < this._nebulaSprites.length; i++) {
      this._nebulaSprites[i].material.rotation += dt * 0.01 * (i % 2 === 0 ? 1 : -1);
    }
