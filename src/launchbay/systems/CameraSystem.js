...
export class CameraSystem {
	...
	update(dt, elapsed) {
		...
		if (this._focus) {
			...
			const bob =
				Math.sin(elapsed * IDLE_BOB_SPEED) *
				(IDLE_HEIGHT_BOB * 0.4)
		...
		this.camera.position.x = damp(
			...
			this._desiredPosition.x,
			DAMP_LAMBDA,
			dt
		)
		this.camera.position.y = damp(
			this.camera.position.y,
			this._desiredPosition.y,
			DAMP_LAMBDA,
			dt
		)
		this.camera.position.z = damp(
			this.camera.position.z,
			this._desiredPosition.z,
			DAMP_LAMBDA,
			dt
		)

		this._dampedLookTarget.x = damp(
			this._dampedLookTarget.x,
			targetLook.x,
			DAMP_LAMBDA,
			dt
		)
		this._dampedLookTarget.y = damp(
			this._dampedLookTarget.y,
			targetLook.y,
			DAMP_LAMBDA,
			dt
		)
		this._dampedLookTarget.z = damp(
			this._dampedLookTarget.z,
			targetLook.z,
			DAMP_LAMBDA,
			dt
		)
