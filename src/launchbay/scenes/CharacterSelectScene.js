...
export class CharacterSelectScene {
	...
	setEmphasis(id, multiplier) {
		if (id in this._emphasisTarget)
			this._emphasisTarget[id] = multiplier
	}
	...
	_initEnvironment() {
		...
		const shapes = [
			{
				geo: new THREE.TorusGeometry(1.1, 0.05, 8, 48),
				position: [-6.5, 2.5, -8],
				rotSpeed: 0.05,
			},
			{
		...
		this._decorMeshes = shapes.map(
			({ geo, position, rotSpeed }) => {
	...
	_tick() {
		...
		for (const id in this._rimLightsById) {
			this._emphasis[id] = damp(
				this._emphasis[id],
				this._emphasisTarget[id],
				4,
				dt
			)
			const phase = elapsed * 0.6 + i * 2.1
			const basePulse = 2.6 + Math.sin(phase) * 0.6
			this._rimLightsById[id].intensity =
				basePulse * this._emphasis[id]
			i++
		}
