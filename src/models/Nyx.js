...
export class Nyx extends Character {
	...
	_addTorsoDetails(torso) {
		...
		// solid tent.
		const cloakGeo = new THREE.ConeGeometry(
			0.42,
			1.1,
			16,
			1,
	...
	_playGravityShift() {
		...
		for (let i = 0; i < 12; i++) {
			...
			const toCenter = new THREE.Vector3(0, 1.0, 0.5)
				.sub(start)
				.normalize()
				.multiplyScalar(2.6)
	...
	_playVoidShield() {
		if (!this._shieldGeometry) {
			this._shieldGeometry = new THREE.SphereGeometry(
				0.65,
				20,
				20
			)
	...
	update(delta, options) {
		...
		if (this._voidEffects && this._voidEffects.length) {
			for (
				let i = this._voidEffects.length - 1;
				i >= 0;
				i--
			) {
		...
		if (this._phase) {
			...
			const opacity =
				1 - Math.sin(Math.min(p, 1) * Math.PI) * 0.75
