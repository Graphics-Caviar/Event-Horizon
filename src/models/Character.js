...
export class Character {
  ...
  _createMaterials() {
    return {
      skin: new THREE.MeshStandardMaterial({ color: 0xd8b48f, roughness: 0.8 }),
      suit: new THREE.MeshStandardMaterial({ color: 0x2b2f36, roughness: 0.7, metalness: 0.2 }),
      accent: new THREE.MeshStandardMaterial({ color: 0x4de3ff, emissive: 0x1c6f80, roughness: 0.4 }),
      dark: new THREE.MeshStandardMaterial({ color: 0x14161c, roughness: 0.9 }),
    };
  }
   ...
   */
  _spawnEffectParticle({ position, velocity, life, material, scale = 1 }) {
    if (!Character._particleGeometry) {
      Character._particleGeometry = new THREE.PlaneGeometry(0.12, 0.12);
    }
    material.userData.baseOpacity = material.opacity;
    const mesh = new THREE.Mesh(Character._particleGeometry, material);
    mesh.position.copy(position);
    mesh.scale.setScalar(scale);
    this.effects.add(mesh);
    this._effectParticles.push({ mesh, velocity, life, maxLife: life, material });
  }
  ...
  update(delta, options = {}) {
    ...
    const targetSpeed = this._state === 'run' ? 2 : this._state === 'walk' ? 1 : 0;
    this._speedFactor = THREE.MathUtils.lerp(this._speedFactor, targetSpeed, 1 - Math.pow(0.001, delta));

    if (this._speedFactor > 0.05) {
      this._locomotionPhase += delta * (2 + this._speedFactor * 3);
  ...
  _animateLocomotion(speedFactor) {
    ...
    this.leftKnee.rotation.x = Math.max(0, Math.sin(phase)) * 0.9 * speedFactor;
    this.rightKnee.rotation.x = Math.max(0, -Math.sin(phase)) * 0.9 * speedFactor;

    this.torso.rotation.x = THREE.MathUtils.lerp(0, 0.18, Math.max(0, speedFactor - 1));
  }
  ...
  _animateJump(delta) {
    ...
    if (j.phase === 'crouch') {
      ...
      this.leftKnee.rotation.x = THREE.MathUtils.lerp(0, 0.6, p);
      this.rightKnee.rotation.x = THREE.MathUtils.lerp(0, 0.6, p);
      if (p >= 1) { j.phase = 'rise'; j.t = 0; }
    } else if (j.phase === 'rise') {
      const p = Math.min(j.t / 0.25, 1);
      this.hips.scale.y = THREE.MathUtils.lerp(0.85, 1.15, p);
      this.leftKnee.rotation.x = THREE.MathUtils.lerp(0.6, 0, p);
      this.rightKnee.rotation.x = THREE.MathUtils.lerp(0.6, 0, p);
      this.leftShoulder.rotation.x = THREE.MathUtils.lerp(0, -0.6, p);
      this.rightShoulder.rotation.x = THREE.MathUtils.lerp(0, -0.6, p);
      if (p >= 1) { j.phase = 'settle'; j.t = 0; }
    } else if (j.phase === 'settle') {
      const p = Math.min(j.t / 0.2, 1);
      this.hips.scale.y = THREE.MathUtils.lerp(1.15, 1, p);
      this.leftShoulder.rotation.x = THREE.MathUtils.lerp(-0.6, 0, p);
      this.rightShoulder.rotation.x = THREE.MathUtils.lerp(-0.6, 0, p);
  ...
  _updateEffectParticles(delta, camera) {
    for (let i = this._effectParticles.length - 1; i >= 0; i--) {
      ...
      p.life -= delta;
      p.material.opacity = Math.max(0, p.life / p.maxLife) * p.material.userData.baseOpacity;
      if (p.life <= 0) {
