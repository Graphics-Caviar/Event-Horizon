...
export class Spaceship {
	...
	_createMaterials() {
		return {
			...
			navLightWhite: new THREE.MeshBasicMaterial({
				color: 0xffffff,
			}),
	...
	_buildCockpit() {
		...
		const positions = new Float32Array([
			-w / 2,
			baseH,
			-len * 0.35, //  0: rear-left base
			w / 2,
			baseH,
			-len * 0.35, //  1: rear-right base
			0,
			baseH,
			len * 0.65, //  2: front tip (nose-ward)
			0,
			apexH,
			-len * 0.15, //  3: apex (canopy peak)
		])
		const indices = [
			0,
			1,
	...
	_buildLights() {
		...
		this.navLightRight = new THREE.Mesh(
			geo,
			this.materials.navLightGreen
		)
		this.navLightRight.position.set(2.55, -0.1, 0.35)
		this.bankPivot.add(this.navLightRight)

		this.navLightLeft = new THREE.Mesh(
			geo,
			this.materials.navLightRed
		)
		this.navLightLeft.position.set(-2.55, -0.1, 0.35)
		this.bankPivot.add(this.navLightLeft)

		this.strobeLight = new THREE.Mesh(
			geo,
			this.materials.navLightWhite
		)
	...
	update(delta, { throttle = 0, turnInput = 0 } = {}) {
		...
		this._bankAngle = THREE.MathUtils.lerp(
			this._bankAngle,
			this._targetBank,
			bankSmoothing
		)
		...
		for (const engine of this.engines) {
			...
			engine.coreMaterial.uniforms.uIntensity.value =
				intensity
		...
		this._gearProgress = THREE.MathUtils.lerp(
			this._gearProgress,
			gearTarget,
			gearSmoothing
		)
		...
		this.navLightRight.visible =
			Math.floor(this._time * 2) % 2 === 0
