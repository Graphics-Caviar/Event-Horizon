...
export class Kai extends Character {
  _createMaterials() {
    ...
    return Object.assign(base, {
      suit: new THREE.MeshStandardMaterial({ color: 0x3a4536, roughness: 0.85, metalness: 0.05 }),
      vest: new THREE.MeshStandardMaterial({ color: 0x51483c, roughness: 0.8 }),
      accent: new THREE.MeshStandardMaterial({ color: 0x7dffa0, emissive: 0x1f6b34, emissiveIntensity: 1.1, roughness: 0.35 }),
      hair: new THREE.MeshStandardMaterial({ color: 0x241a12, roughness: 0.8 }),
      metal: new THREE.MeshStandardMaterial({ color: 0x6b6d6a, roughness: 0.5, metalness: 0.6 }),
    });
  }

  _addHeadDetails(head) {
    const hairGeo = new THREE.SphereGeometry(0.17, 12, 12, 0, Math.PI * 2, 0, Math.PI * 0.5);
  ...
  _addTorsoDetails(torso) {
    ...
    const vestGeo = new THREE.CylinderGeometry(0.28, 0.24, 0.42, 8, 1, true);
    ...
    for (const x of [-0.15, 0, 0.15]) {
      const pouch = new THREE.Mesh(pouchGeo, this.materials.metal);
  ...
  _addEquipment(equipment, backpack) {
    ...
    const toolGeo = new THREE.CylinderGeometry(0.015, 0.015, 0.22, 6);
  ...
  playAbility() {
    if (!this._droneGeometry) {
      this._droneGeometry = new THREE.OctahedronGeometry(0.09, 0);
    }
    const material = new THREE.MeshStandardMaterial({
      color: 0x1a2318, emissive: 0x2fbf5a, emissiveIntensity: 1.4, roughness: 0.4,
    });
    ...
    this._drones.push({ mesh: drone, material, life: 2.2, baseY: 1.5 });

    for (let i = 0; i < 10; i++) {
      const sparkMaterial = new THREE.MeshBasicMaterial({
        color: 0x7dffa0, transparent: true, opacity: 0.9,
        blending: THREE.AdditiveBlending, depthWrite: false,
      });
      const position = new THREE.Vector3(0.4 + (Math.random() - 0.5) * 0.2, 1.4 + Math.random() * 0.3, 0.1);
      const velocity = new THREE.Vector3((Math.random() - 0.5) * 1.2, (Math.random() - 0.3) * 1.0, (Math.random() - 0.5) * 1.2);
      this._spawnEffectParticle({ position, velocity, life: 0.3 + Math.random() * 0.3, material: sparkMaterial, scale: 0.06 });
    }
  }
  ...
  update(delta, options) {
    ...
    if (this._drones && this._drones.length) {
      for (let i = this._drones.length - 1; i >= 0; i--) {
        ...
        d.mesh.position.y = d.baseY + Math.sin(d.time * 3) * 0.08;
        d.life -= delta;
        if (d.life <= 0) {
